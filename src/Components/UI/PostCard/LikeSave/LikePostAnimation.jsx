...
const LikePostAnimation = ({ post, liked, likePost }) => {
  ...
  return (
    <div
      ...
      className="relative cursor-pointer w-1/2 bg-black overflow-y-scroll flex flex-col justify-center img-wrapper"
    >
      <img className="w-full bg-contain peer" src={post?.imageUrl} alt="" />
      <Favorite
        fontSize="large"
        className={`${
          heartActive ? "heart" : ""
      ...
      />
      <p className="text-white text-sm font-medium p-2 transition-all duration-500 opacity-0 peer-hover:opacity-100">{post.description}</p>
